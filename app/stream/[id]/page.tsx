...
export default function StreamPage() {
  ...
  useEffect(() => {
    if (!autoRefreshInterval || autoRefreshInterval < MIN_REFRESH_INTERVAL_S)
      return;
      ...
      if (remaining <= 0) {
        ...
        if (publicKey && streamAddress) {
          void getWithdrawable(publicKey, streamAddress)
            .then((amount) => {
              if (active) setWithdrawable(amount);
            })
            .catch(() => {
              /* keep the last known balance on refresh failure */
            });
        }
        return;
      }
  ...
  if (!connected)
    return (
      <div className="max-w-2xl mx-auto px-4 py-10">
        <Link
          href="/streams"
          className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-black dark:hover:text-white mb-6"
        >
  ...
  return (
    ...
    >
      ...
      <Link
        href="/streams"
        className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-black dark:hover:text-white mb-6 print:hidden"
      >
        <ArrowLeft className="w-3.5 h-3.5" /> All streams
      </Link>
      ...
      {streamAddress && (
        <div className="mb-6 print:hidden">
          <StreamNoteEditor
            note={noteHook.note}
            isEditing={noteHook.isEditing}
            onStartEditing={noteHook.startEditing}
            onStopEditing={noteHook.stopEditing}
            onSave={noteHook.updateNote}
            onClear={noteHook.clearNote}
          />
        </div>
      )}
      ...
      {status === "paused" && (
        <Card className="mb-6 text-center">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
            Stream is paused — balance frozen
          </p>
        </Card>
      )}
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            <tr>
              ...
              <td className="py-2.5 font-mono text-black dark:text-white text-right">
                {formatTimestamp(info.startTime)}
              </td>
            </tr>
            <tr>
              <td className="py-2.5 text-gray-400 dark:text-gray-500 w-40">
            ...
            <tr>
              ...
              <td className="py-2.5 font-mono text-black dark:text-white text-right">
                {info.clawbackEnabled ? "Enabled" : "Disabled"}
              </td>
            </tr>
          </tbody>
        </table>
